...
A real external call can occur only when runtime wiring chooses a real adapter and provides all required credentials/configuration, explicit approval/policy gates, explicit operation gates, and an injected transport. Normal tests use fake transports and do not call the network. Integration results must keep `realNetworkCall: false` until a gated real adapter actually invokes its injected transport. Gated real integration results expose sorted execute blockers for missing env/manual actions, transport, network, operation, policy, safety, or unclassified manual-action conditions; `safeToRun: false` is compatibility evidence, not a standalone blocker reason. Blocker details are restricted to sanitized metadata such as env var names, action/resource ids, boolean gate flags, and safe paths. `github.pr` default Worker Runner output remains manual-action/no-network and persists only a reviewable PR preview.
