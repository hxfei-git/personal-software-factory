...
Hub Web is not the source of truth for Mission state. It reads from the Orchestrator and sends protected dry-run requests. It does not run shell commands, read or write the filesystem, access the database directly, reset demo data, delete records, or call GitHub, Coolify, Uptime Kuma, Plane, or other external providers directly.
