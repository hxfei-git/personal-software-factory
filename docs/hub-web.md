...
Hub Web is the local operator console for Personal Software Factory. The current implementation is a React/Vite app that reads Orchestrator API data and triggers only protected dry-run actions through the Orchestrator API.
...
Hub Web is not the source of truth for Mission state. It reads from the Orchestrator and sends protected dry-run requests. It does not run shell commands, read or write the filesystem, access the database directly, reset demo data, delete records, or call GitHub, Coolify, Uptime Kuma, Plane, or other external providers directly. Real-mode status, links, approvals, and retention metadata are rendered only from `GET /missions/:id/summary`; Hub does not inspect the database or filesystem to produce those views.
